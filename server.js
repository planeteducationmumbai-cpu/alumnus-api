...
CREATE TABLE IF NOT EXISTS competitions (
  ...
  title TEXT, prize TEXT, scholarship_pct TEXT,
  days_left INTEGER, deadline TEXT, color TEXT,
  stream TEXT, scope TEXT, ages TEXT, comp_type TEXT,
  overview TEXT, process TEXT, criteria TEXT,
  how_to_apply TEXT, contact_email TEXT, external_link TEXT,
...
app.get("/api/competitions", (_req, res) =>
  res.json(db.prepare("SELECT * FROM competitions ORDER BY featured DESC, days_left ASC").all()));

app.get("/api/competitions/:id", (req, res) => {
  const c = db.prepare("SELECT * FROM competitions WHERE id=?").get(req.params.id);
  if (!c) return res.status(404).json({ error: "not found" });
  res.json(c);
});

app.post("/api/admin/competitions", adminAuth, (req, res) => {
  const { title, prize, scholarship_pct, days_left, deadline, color, stream, scope, ages, comp_type, overview, process, criteria, how_to_apply, contact_email, external_link, featured } = req.body || {};
  if (!title) return res.status(400).json({ error: "title required" });
  const r = db.prepare(
    `INSERT INTO competitions (title,prize,scholarship_pct,days_left,deadline,color,stream,scope,ages,comp_type,overview,process,criteria,how_to_apply,contact_email,external_link,featured)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(title, prize||"", scholarship_pct||"", days_left||30, deadline||"", color||"#13294B", stream||"", scope||"", ages||"", comp_type||"", overview||"", process||"", criteria||"", how_to_apply||"", contact_email||"", external_link||"", featured?1:0);
  res.json({ id: r.lastInsertRowid });
});
...
app.put("/api/admin/competitions/:id", adminAuth, (req, res) => {
  const { title, prize, scholarship_pct, days_left, deadline, color, stream, scope, ages, comp_type, overview, process, criteria, how_to_apply, contact_email, external_link, featured } = req.body || {};
  db.prepare(
    `UPDATE competitions SET title=?,prize=?,scholarship_pct=?,days_left=?,deadline=?,color=?,stream=?,scope=?,ages=?,comp_type=?,overview=?,process=?,criteria=?,how_to_apply=?,contact_email=?,external_link=?,featured=? WHERE id=?`
  ).run(title, prize||"", scholarship_pct||"", days_left||30, deadline||"", color||"#13294B", stream||"", scope||"", ages||"", comp_type||"", overview||"", process||"", criteria||"", how_to_apply||"", contact_email||"", external_link||"", featured?1:0, req.params.id);
  res.json({ ok: true });
});
...
function autoSeed() {
  ...
  const comps = [
    ["Scholarship Challenge 2026", "100% Tuition Fee", "100%", 12, "2026-07-15", "#13294B", "All Streams", "National", "18-25", "Submission", "A flagship national scholarship competition open to graduates across all streams.", "Submit a research paper or project proposal in three rounds.", "Open to all graduates under 25 years of age.", "Submit via the registration portal before the deadline.", "", "", 1],
    ["Data Science Arena 2026", "Full Scholarship + Internship", "100%", 21, "2026-07-25", "#214A7B", "Technology", "National", "18-28", "Competition", "A national data science competition with full scholarship and internship prizes for the top performers.", "Individual or team entry with three elimination rounds.", "Open to undergraduates and postgraduates in any discipline.", "Register and submit your data analysis project.", "", "", 1],
  ];
  comps.forEach(c => db.prepare(
    `INSERT OR IGNORE INTO competitions (title,prize,scholarship_pct,days_left,deadline,color,stream,scope,ages,comp_type,overview,process,criteria,how_to_apply,contact_email,external_link,featured) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(...c));
  const interns = [
